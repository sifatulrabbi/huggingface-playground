...
try:
    ...
    model = Dia.from_pretrained(
        "nari-labs/Dia-1.6B-0626", compute_dtype=dtype, device=device
    )
...
def run_inference(
    ...
    with contextlib.redirect_stdout(console_output_buffer):
        ...
        if (
            audio_prompt_input
            and audio_prompt_text_input
            and not audio_prompt_text_input.isspace()
        ):
            text_input = audio_prompt_text_input + "\n" + text_input
            text_input = text_input.strip()

        if audio_prompt_input and (
            not audio_prompt_text_input or audio_prompt_text_input.isspace()
        ):
